---
title: Runbook
---

import Link from '@docusaurus/Link';

# Product configurations

1.  Calm VM 4.2.0 on PC7.3
2.  Infrastructure cluster on AOS 7.3 on PC7.3

# What is a Runbook

Blueprint describes all elements of an applications.  A blueprint can launch either a single VM or multi-VM application such as 
such as web, application and database.  An application manages the lifecycle within the web, application and database.

Runbook describes a series of tasks to be executed regularly across endpoints.  Both physical server or virtual machine with a 
network IP address is considered an endpoint.  Runbook can traverse across applications.  Runbooks can be hooked to a scheduler to be 
executed on a regular basis.

![](images/Runbook-Action.png)

Some of the use cases are 

1.  <Link href="https://portal.nutanix.com/page/documents/details?targetId=Self-Service-Admin-Operations-Guide-v4_2_0:nuc-app-mgmt-ndb-with-runbooks-c.html" target="_blank" rel="noopener">Nutanix Database Service (NDB) integration with Self Service Runbook</Link>


2.  <Link href="https://portal.nutanix.com/page/documents/details?targetId=Self-Service-Admin-Operations-Guide-v4_2_0:nuc-app-mgmt-multi-tenant-capability-sp-c.html" target="_blank" rel="noopener">Service Provider Pack (Multi-Tenanted capabilities for Runbook)</Link>

